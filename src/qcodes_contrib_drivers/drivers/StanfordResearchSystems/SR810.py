...
class ChannelTrace(ParameterWithSetpoints):
    ...
    def __init__(self, name: str, channel: int, **kwargs: Any) -> None:
        ...
        if channel not in self._valid_channels:
            raise ValueError(
                "Invalid channel specifier. SR810 only has channels 1 and 2."
            )

        if not isinstance(self.root_instrument, SR810):
            raise ValueError(
                "Invalid parent instrument. ChannelBuffer can only live on an SR810."
            )
    ...
    def get_raw(self) -> ParamRawDataType:
        ...
        if N == 0:
            raise ValueError(
                "No points stored in SR810 data buffer. Can not poll anything."
            )
